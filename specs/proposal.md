...
The API includes a `FilesystemTree` class encapsulating the structure of a random filesystem tree.  
FilesystemTrees are constructed with the following parameters:
 ...
 - node_file_delta: Random varation +/- of the number of files in each non-leaf dir.
 - node_dir_avg: The average number of subdirs in non-leaf dirs.
 - node_dir_delta: Random variation +/- of the number of files in each non-leaf dir.

FilesystemTree instances have the following methods:
 ...
 - apply(path: pathlibPath): Create files and directories under `path` to materialise the FilesystemTree.
